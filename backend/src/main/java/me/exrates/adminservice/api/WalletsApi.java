package me.exrates.adminservice.api;
...
import me.exrates.adminservice.exceptions.WalletsApiException;
...
public class WalletsApi {
    ...
    public List<BalanceDto> getBalancesFromApi() {
        ...
        try {
            responseEntity = restTemplate.exchange(url, HttpMethod.GET, new HttpEntity<>(getAuthHeaders()), WalletsData[].class);
    ...
    public Map<String, BigDecimal> getReservedBalancesFromApi() {
        ...
        try {
            responseEntity = restTemplate.exchange(url + "/balance/reserved", HttpMethod.GET, new HttpEntity<>(getAuthHeaders()), ReservedWalletsData.class);
            if (responseEntity.getStatusCodeValue() != 200) {
                throw new WalletsApiException("Wallets server is not available");
            }
        } catch (Exception ex) {
    ...
    public BigDecimal getBalanceByCurrencyAndWallet(String currencySymbol, String walletAddress) {
        ...
        try {
            responseEntity = restTemplate.exchange(builder.toUriString(), HttpMethod.GET, new HttpEntity<>(getAuthHeaders()), WalletBalanceData.class);
            if (responseEntity.getStatusCodeValue() != 200) {
                throw new WalletsApiException("Wallets server is not available");
            }
        } catch (Exception ex) {
    ...
    public boolean addReservedWallet(String currencySymbol, String walletAddress) {
        ...
        try {
            responseEntity = restTemplate.exchange(url + "/add", HttpMethod.POST, new HttpEntity<>(builder.build(), getAuthHeaders()), Boolean.class);
            if (responseEntity.getStatusCodeValue() != 200) {
                throw new WalletsApiException("Wallets server is not available");
            }
        } catch (Exception ex) {
    ...
    public boolean deleteReservedWallet(String currencySymbol, String walletAddress) {
        ...
        ResponseEntity<Boolean> responseEntity;
        try {
            responseEntity = restTemplate.exchange(url + "/delete", HttpMethod.DELETE, new HttpEntity<>(builder.build(), getAuthHeaders()), Boolean.class);
            if (responseEntity.getStatusCodeValue() != 200) {
                throw new WalletsApiException("Wallets server is not available");
            }
        } catch (Exception ex) {
